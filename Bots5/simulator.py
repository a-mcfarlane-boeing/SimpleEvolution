...
class Simulator:
    ...
    def run(self):
        ...
        while self.status:
            ...
            #self.status = botCount > self.minNumOfBots
            ...
            if (self.runTime - last_print_time) >= self.frameInterval:
                ...
                #for object in self.simObjects:
                #    if "bot" in object.attributes.name:
                #        self.brainWindow.connected_brain = object.net
                #        self.brainWindow.window.title(object.attributes.name)
    ...
    def endOfSimulation(self):
                ...
                for bot in bestBots:
                    if len(bestBots) == 0 :
                        ...
                        if object.sameSpecies(bot):
                            if object.total_rewards_collected >= bot.total_rewards_collected and object.attributes.generation != 0:
                                bot = object
                        else:
                            if object.total_rewards_collected >= 3 and object.attributes.generation != 0:
                                bestBots.append(object)
        ...
        #record the results of the simulation
        for bot in bestBots:
            text = "the bot which colllected the most rewards was "+bot.attributes.name + " RPM: "+str(bot.total_rewards_collected/(bot.age/60.0))+" Gen: "+ str(bot.attributes.generation)+" with "+str(bot.total_rewards_collected)+" Max Speed: "+str(bot.attributes.maxSpeed)
            record = open("record.txt","a")
            record.write(text+"\n")
            record.close()
            #show results to the screen
...
def main():
    ...
    initial_num_of_bots = 30

    import bot
    import reward
    plane = world.World(30,30)
    simulator = Simulator(5,1,plane)

    for i in range(initial_num_of_bots):
