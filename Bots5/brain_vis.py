...
class BrainDisplay:
    ...
    def update(self):
        ...
        for neuron in self.connected_brain.neurons:
            ...
            self.neurons.append(self._createCircle(neuron.location[0],neuron.location[1],valueToNeuronRadius(neuron.output),colour))
    ...
    def _createCircle(self, x_pos, y_pos, radius, colour):
        ...
        circle_object = self.canvas.create_oval(UL_x,UL_y,LR_x,LR_y,fill=colour)
        return circle_object

    def _changeNeuronsSizes(self):
        """
        Changes the size of a circle to a new radius based on output value
        """
        for this_neuron in self.neurons:
            for brain_neuron in self.connected_brain.neurons:
                if this_neuron['name'] == brain_neuron.name:
                    x = this_neuron['x']
                    y = this_neuron['y']

                    new_radius = valueToNeuronRadius(brain_neuron.output)
                    ...
                    self.canvas.coords(this_neuron["circle"]["object"], UL_x, UL_y, LR_x, LR_y)
