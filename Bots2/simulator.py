...
while i < number_of_bots:
    ...
    bots[i].net.loadBrain("brains/starter_brain.txt")
...
while j < number_of_bots:
    ...
    while k < bots[-1].net.num_of_neurons:
        ...
        while l < bots[-1].net.num_of_connections + 1:
            ...
            difference = x - change
            if random.random() < (number_of_bots/100.0):
                #mutation
    ...
    if difference >= frame_interval:
        ...
        if number_of_bots <= 1 or simulation_elapsed_time >= time_limit:
            ...
            while i < len(allBots):
                ...
                if carry_factor >= max_carry_factor and allBots[i].age >=100 and allBots[i].name != "bot0":
